...
class MultiModalFeatureExtractor:
    def __init__(self, insightface_name: str = "buffalo_l", face_det_size: tuple = (320, 320), device: str = "cuda"):
        ...
        self.face_analyzer.prepare(ctx_id=0 if torch.cuda.is_available() else -1, det_size=face_det_size)
...
class YOLOPoseFixedIDReID3DPipeline:
    def __init__(
        ...
        _pose = pose_model_path or self.config.get("model.pose_model")
        _intrin = intrinsics_path or self.config.get("camera.intrinsics_path")
        _extrin = extrinsics_path or self.config.get("camera.extrinsics_path")
        _v2c = view_to_camera or self.config.view_to_camera
        _num = self.config.get("reid.num_players", 6)
        _sw = self.config.get("reid.smooth_window", 5)
        _mm = self.config.get("reid.max_missing_frames", 10)
        _vw = self.config.get("reid.velocity_window", 5)
        _ifname = self.config.get("model.insightface_name", "buffalo_l")
        _fdet = tuple(self.config.get("reid.face_det_size", [320, 320]))
        self.skeleton_connections = self.config.skeleton_connections
        self.player_colors = self.config.player_colors_rgb
        print("加载YOLO-Pose模型...")
        self.pose_model = YOLO(_pose)
        print("加载相机参数...")
        self.cameras = CameraParamLoader(_intrin, _extrin).load()
        self.view_to_camera = _v2c
        self.triangulator = Triangulator(self.cameras, self.view_to_camera)
        print("初始化多模态特征提取器...")
        self.feature_extractor = MultiModalFeatureExtractor(insightface_name=_ifname, face_det_size=_fdet)
        self.reid = PerViewFixedIDReID(num_players=_num)
        self.smoother = TrajectorySmoother(smooth_window=_sw, max_missing=_mm, velocity_window=_vw)
        ...
        for frame_num in range(start_frame, end_frame):
            ...
            for view_name, frame in frames.items():
                ...
                for det in all_dets.get(view_name, []):
                    gid = matching.get(view_name, {}).get(id(det))
                    if gid:
                        color = self.player_colors[(gid - 1) % len(self.player_colors)]
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="YOLO-Pose + ReID + 3D投影")
    ...
    output_dir = cfg.get("output.reid_3d_dir")
    os.makedirs(output_dir, exist_ok=True)
    
    pipeline = YOLOPoseFixedIDReID3DPipeline(config=cfg)
    start = args.start_frame if args.start_frame is not None else cfg.get("trajectory.start_frame", 0)
    end = args.end_frame if args.end_frame is not None else start + int(cfg.get("trajectory.process_seconds", 30) * cfg.get("trajectory.fps", 30))
    results = pipeline.process_multiple_videos(existing, output_dir, start, end)
