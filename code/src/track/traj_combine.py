...
class SegmentTrajectoryMatcher:
    ...
    def __init__(
        ...
        # 数据存储
        self.all_json_traj_data: Dict[
            str, Dict[str, Dict[Union[str, int], Any]]
        ] = {}  # 所有加载的JSON轨迹
        self.json_paths: List[str] = []  # 按顺序存储JSON路径
        self.json_index_map: Dict[str, int] = {}  # JSON路径→序号（1,2,3...）

        self.current_merged_traj: Dict[
            str, Dict[Union[str, int], Any]
        ] = {}  # 当前合并池
        self.cumulative_unmatched_traj: Dict[
            str, Dict[Union[str, int], Any]
        ] = {}  # 累计未匹配轨迹
    ...
    def _load_single_json(
        self, reid_json_path: str
    ) -> Dict[str, Dict[Union[str, int], Any]]:
    ...
    # ===================== 计算两条轨迹的平均距离 =====================
    def _calculate_two_traj_distance(
        self, traj1: Dict[Union[str, int], Any], traj2: Dict[Union[str, int], Any]
    ) -> float:
        traj1_frames = {
            int(k): (float(v["x"]), float(v["y"]))
            for k, v in traj1.items()
            if k.isdigit()
        }
        traj2_frames = {
            int(k): (float(v["x"]), float(v["y"]))
            for k, v in traj2.items()
            if k.isdigit()
        }
    ...
    # ===================== 合并两条轨迹（重叠帧坐标取平均） =====================
    def _merge_two_trajectories(
        self,
        traj1: Dict[Union[str, int], Any],
        traj2: Dict[Union[str, int], Any],
        traj1_id: str,
        traj2_id: str,
        ...
        merged_traj["player_id"] = (
            player1 if player1 == player2 else f"{player1}+{player2}"
        )
        ...
        for frame_str in all_frames:
            ...
            if frame_str in traj1_frames and frame_str in traj2_frames:
                ...
                merged_frame["fusion_note"] = (
                    f"average of {traj1_id} and {traj2_id} (x:{frame1['x']:.2f}/{frame2['x']:.2f}, y:{frame1['y']:.2f}/{frame2['y']:.2f})"
                )
    ...
    def _save_round_result(self, round_idx: int) -> str:
        ...
        merged_output = {"final_merged_finished_trajectories": self.current_merged_traj}
        with open(merged_json_path, "w", encoding="utf-8") as f:
            json.dump(merged_output, f, ensure_ascii=False, indent=4)

        # 3. 保存该轮未匹配轨迹JSON
        unmatched_json_path = os.path.join(
            round_folder_path, "unmatched_trajectories.json"
        )
        unmatched_output = {
        ...
        # 4. 绘制并保存该轮合并轨迹图
        self._plot_traj_to_folder(
            self.current_merged_traj,
            round_folder_path,
    ...
    # ===================== 绘制轨迹图并保存到指定文件夹 =====================
    def _plot_traj_to_folder(
        self,
        traj_data: Dict[str, Dict[Union[str, int], Any]],
        folder_path: str,
        fig_name: str,
        title: str,
    ) -> str:
            ...
            for pid in common_pids:
                ...
                for c_tid, n_tid, dist in distance_pairs:
                    ...
                    if dist < self.match_threshold:
                        ...
                        merged_tid, merged_traj = self._merge_two_trajectories(
                            c_traj, n_traj, c_tid, n_tid
                        )
                        ...
                        print(
                            f"    匹配成功：player_id={pid} | {c_tid} ↔ {n_tid} | 距离={dist:.2f}米 → 合并为{merged_tid}"
                        )
                    else:
                        print(
                            f"    匹配失败：player_id={pid} | {c_tid} ↔ {n_tid} | 距离={dist:.2f}米（阈值{self.match_threshold}米）"
                        )

        # 4. 收集该轮未匹配的轨迹，加入累计未匹配池
        round_unmatched_traj = {
            tid: traj
            for tid, traj in next_traj_data.items()
            if tid not in matched_next_traj_ids
        }
    ...
    def run_serial_merge(self) -> None:
        ...
        # 保存最终合并结果
        final_output = {"final_merged_finished_trajectories": self.current_merged_traj}
    ...
    def print_merge_summary(self) -> None:
        ...
        for round_info in self.merge_rounds:
            print(
                f"  第{round_info['round_idx']}轮：文件夹={round_info['folder_name']} | 合并轨迹={round_info['merged_traj_count']}条 | 未匹配轨迹={round_info['unmatched_traj_count']}条"
            )
