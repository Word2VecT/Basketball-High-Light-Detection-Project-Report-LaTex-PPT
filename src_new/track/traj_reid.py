...
class TrajectoryReIDVisualizer:
    ...
    def __init__(
        ...
        self.REFERENCE_FACES_DIR = "/data/ljy23/project/code/assets/ref1"  # 参考图片文件夹（人脸/Qwen共用）
        ...
        # ===================== 模式配置 + 模型初始化 =====================
        # 3. 模式校验
        self.operation_mode = operation_mode if operation_mode in ["face", "qwen", "siglip", "insightface"] else "face"
        ...
        elif self.operation_mode == "insightface":
            # InsightFace模式：使用轻量级模型，强制使用 CPU 避免 CUDA 环境问题
    ...
    def _match_roi_face(
        ...
        """Face 或 InsightFace 模式：在 person_roi 中检测人脸并与参考库匹配。"""
        matches: List[Dict] = []
    ...
    def match_single_traj_to_person(self, traj_id: str, traj_data: Dict[int, Dict]) -> str:
        """匹配单条轨迹到具体球员（统一 face/qwen/siglip/insightface 模式）。"""
        # 前置检查
        if self.operation_mode in ["face", "insightface"]:
            ...
            for view_name, view_boxes in view_groups.items():
                ...
                for box_item in view_boxes:
                    ...
                    if person_roi is None:
                        continue
                    if self.operation_mode in ["face", "insightface"]:
                        box_matches.extend(self._match_roi_face(person_roi, frame, coords))
                    else:
                        box_matches.extend(self._match_roi_model(person_roi))
        ...
        if self.operation_mode == "face":
            ...
            return f"{self.traj_player_mapping[traj_id]} (insightface) (占比: {ratio:.2%})"
    ...
    def generate_merged_json_with_player_id(self) -> None:
        ...
        final_json = {
            ...
            "face_detection_mode": self.FACE_DETECTION_MODE if self.operation_mode == "face" else "insightface" if self.operation_mode == "insightface" else None,
    ...
    def run_direct_statistics(self) -> Dict[str, str]:
        """直接统计每帧的 ID，使用之前的 ReID 结果进行统计。"""
        t0 = time.time()
        try:
            logger.info(f"[traj_reid] 开始直接统计 | 模式: {self.operation_mode} | 帧范围: {self.START_FRAME}~{self.MAX_PROCESS_FRAMES}")
            print("\n🚀 开始直接统计轨迹ID...")
            print(f"   模式: {self.operation_mode.upper()}")
            print(f"   帧范围: {self.START_FRAME} ~ {self.MAX_PROCESS_FRAMES}")
            print(f"   总帧数: {self.MAX_PROCESS_FRAMES - self.START_FRAME}")
            ...
            total = len(self.traj_player_mapping)
            logger.info(
                f"[traj_reid] 直接统计完成 | 轨迹 {total} 条 | 匹配 {matched} | 未匹配 {total - matched} | 耗时 {elapsed:.1f}s"
            )
