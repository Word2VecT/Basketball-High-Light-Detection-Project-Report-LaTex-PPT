...
class TrajectoryVideoStitcher:
    ...
    # ===================== 新增：多帧断帧补齐方法 =====================
    def _fill_missing_frames_for_traj(self, traj_frames: Dict[int, Tuple[float, float]]) -> Dict[int, Tuple[float, float]]:
        """
        补齐单条轨迹中连续的多帧断帧（线性插值）
    ...
    def _draw_trajectory_legend(self, frame: np.ndarray, active_players: List[Dict], current_frame: int) -> None:
        ...
        for i, player_info in enumerate(active_players):
            ...
            cv2.rectangle(
                frame,
                (start_x + 10, color_block_y),
                (start_x + 30, color_block_y + 15),
                color,
                -1
            )
