...
class TrajectoryVideoStitcher:
    ...
    def __init__(
        ...
        # 全局轨迹数据
        self.global_traj_data: Dict[str, Dict[int, Tuple[float, float, str]]] = {}  # traj_id: {frame: (x, y, video_path)}
    ...
    def _load_global_json_data(self) -> None:
        ...
        for traj_id, traj_info in traj_root.items():
            ...
            if frame_coords:
                ...
                # 提取player_id
                self.global_player_id_map[traj_id] = traj_info.get("player_id", "未匹配")
                total_traj_count += 1
    ...
    def _filter_traj_data_for_video(self, target_video_path: str) -> None:
        ...
        for traj_id, frame_data in self.global_traj_data.items():
            ...
            if filtered_frames:
                ...
                self.player_id_map[traj_id] = self.global_player_id_map.get(traj_id, "未匹配")
                traj_count += 1
    ...
    def _draw_right_frame(self, current_frame: int) -> np.ndarray:
        ...
        cv2.putText(frame, f"Half Court | Frame: {current_frame} ({self.start_frame}-{self.maxframe})", 
                    (self.LEGEND_MARGIN, self.LEGEND_MARGIN + 30),
                    cv2.FONT_HERSHEY_SIMPLEX, self.FONT_SCALE, (0,0,0), self.FONT_THICKNESS)
        cv2.putText(frame, f"Single JSON: {len(self.traj_data)} tracks | Output: {self.output_root_dir}", 
    ...
    def _stitch_frames(self, left_frame: np.ndarray, right_frame: np.ndarray, left_width_scaled: int, stitch_height: int) -> np.ndarray:
        """拼接左右帧"""
        left_resized = cv2.resize(left_frame, (left_width_scaled, stitch_height), interpolation=cv2.INTER_CUBIC)
        stitch_frame = np.hstack((left_resized, right_frame))
        return stitch_frame

    # ===================== 生成单个视频拼接结果 =====================
    def _generate_single_stitch_video(self, video_path: str, video_index: int) -> Optional[str]:
        """为单个视频生成拼接视频"""
        try:
            ...
            # 1. 初始化单个视频的基础信息
            self.left_width, self.left_height, left_fps, maxframe = self._load_input_video_info(video_path)
    ...
    def batch_generate_stitch_videos(self) -> List[Optional[str]]:
        ...
        print(f"成功生成：{len([p for p in self.generated_video_paths if p is not None])}个")
