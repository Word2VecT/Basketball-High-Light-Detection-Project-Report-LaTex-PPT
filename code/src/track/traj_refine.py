...
class TrajRefine:
    ...
    def __init__(
        ...
        self.overlap_resolution = overlap_resolution if overlap_resolution in ["higher_confidence", "smooth_transition"] else "higher_confidence"
        ...
        for traj_id, traj_data in self.original_trajectories.items():
            ...
            for frame_str, frame_info in traj_data.items():
                ...
                if "box" in frame_info and isinstance(frame_info["box"], list) and len(frame_info["box"]) > 0:
                    ...
                    if "box_data" in first_box and isinstance(first_box["box_data"], list) and len(first_box["box_data"]) == 4:
    ...
    def connect_two_trajs_with_gap(self, traj1: Dict, traj2: Dict) -> Tuple[Dict, Dict, Dict]:
        ...
        try:
            ...
            return traj1, traj2, {
                "connected": False,
                "gap_frames": 0,
                "reason": "Frame number format error"
            }
    ...
    def can_merge_two_trajs(self, traj1: Dict, traj2: Dict) -> Tuple[bool, str, Dict]:
        ...
        if overlap_start <= overlap_end:
            ...
            overlap_frames = overlap_end - overlap_start + 1
            if overlap_frames > self.max_overlap_frames:
                return False, f"Overlap of {overlap_frames} frames exceeds maximum allowed {self.max_overlap_frames} frames", {
                    "type": "overlap_exceed_max",
                    "overlap_frames": overlap_frames
                }
            else:
                return True, f"Overlap of {overlap_frames} frames within allowed range, can be processed", {
                    "type": "overlap_within_limit",
                    "overlap_frames": overlap_frames
                }
        else:
            ...
            gap = traj2_start - traj1_end - 1
            if gap > self.max_gap_for_connection:
                return False, f"Gap of {gap} frames exceeds maximum allowed {self.max_gap_for_connection} frames", {
                    "type": "gap_exceed_max",
                    "gap_frames": gap
                }
            elif gap < 0:
                # Shouldn't happen
                return False, f"Calculation error: gap={gap}", {
                    "type": "calculation_error",
                    "gap_frames": gap
                }
            else:
                return True, f"Gap of {gap} frames within allowed range, can be connected", {
                    "type": "gap_within_limit",
                    "gap_frames": gap
                }
            ...
            if gap < 0:
                ...
                if connect_stats["connected"]:
                    ...
                    if 'original_traj_ids' not in merged_traj:
                        merged_traj['original_traj_ids'] = list(set(
                            traj1_after.get('original_traj_ids', [traj1_after['traj_id']]) +
                            traj2_after.get('original_traj_ids', [traj2_after['traj_id']])
                        ))
                    return merged_traj
    ...
    def connect_all_trajectories(self) -> None:
        ...
        for stat in connection_stats:
            print(f"  {stat['connected_traj_id']}: Player {stat['player_id']} | Original {len(stat['original_trajectories'])} | Interpolated {stat['interpolated_frames']} frames")
        print("-" * 80)
    ...
    def rebuild_json_structure(self, include_unconnected: bool = True) -> Dict:
        ...
        refined_json = {
            "final_merged_finished_trajectories": refined_trajectories
        }
        ...
        refined_json["refine_info"] = {
            ...
            "connected_trajectories_count": len([t for t in self.connected_trajectories.values() if t.get('is_connected', False)]),
    ...
    def visualize_connection_results(self) -> None:
        ...
        for traj_id, traj_info in self.connected_trajectories.items():
            ...
            for i, (frame, x, y, frame_data) in enumerate(points):
                ...
                if i == 0:
                    cv2.putText(canvas, f"Start:{frame}", (x + 5, y - 5), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
                elif i == len(points) - 1:
                    cv2.putText(canvas, f"End:{frame}", (x + 5, y - 5), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
            ...
            cv2.putText(canvas, "Original", (80, legend_y + 20), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 0), 1)
            cv2.circle(canvas, (150, legend_y + 15), 6, (0, 0, 255), -1)
            cv2.putText(canvas, "Interpolated", (160, legend_y + 20), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 0), 1)
            cv2.circle(canvas, (250, legend_y + 15), 5, (255, 165, 0), -1)
            cv2.putText(canvas, "Blended", (260, legend_y + 20), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 0), 1)
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            for player_id in sorted(self.player_trajectories.keys()):
                ...
                for traj in sorted(player_trajs, key=lambda x: x['start_frame']):
                    f.write(f"  {traj['traj_id']}: Frames {traj['start_frame']}-{traj['end_frame']} "
                           f"({traj['end_frame'] - traj['start_frame'] + 1} frames, "
                           f"avg conf: {traj['avg_confidence']:.3f})\n")
                
                # 查找该玩家的处理决策
                player_decisions = [d for d in self.trajectory_connection_decisions 
                ...
                if player_decisions:
                    ...
                    for idx, dec in enumerate(player_decisions, 1):
                        ...
                        if dec.get('decision_type') == 'merged':
                            f.write(f"       ✅ Merged successfully\n")
                            f.write(f"       Reason: {dec.get('reason', 'N/A')}\n")
                        else:
                            f.write(f"       ❌ Not merged\n")
                            f.write(f"       Reason: {dec.get('reason', 'N/A')}\n")
            ...
            if total_connected > 0:
                merged_trajs = [t for t in self.connected_trajectories.values() 
                              if t.get('is_connected', False)]
                
                for traj in sorted(merged_trajs, key=lambda x: x['player_id']):
                    f.write(f"Trajectory: {traj['traj_id']}\n")
                    f.write(f"  Player: {traj['player_id']}\n")
                    f.write(f"  Frame range: {traj['start_frame']}-{traj['end_frame']} "
                           f"({traj['end_frame'] - traj['start_frame'] + 1} frames)\n")
...
def refine_pipe(input_json: str,
                id_json_path: str,
                output_dir: str,
...
if __name__ == "__main__":
    input_json = "/data/ljy23/project/code/test/traj_reid/traj_reid/merged_trajectories_with_player_id_3200-5200frames.json"
    id_json="/data/ljy23/project/code/test/traj_reid/traj_reid/frame_player_ids_3200-5200frames.json"
    refine_pipe(input_json=input_json,
                id_json_path=id_json,
                output_dir="./test/traj_refined"
    )