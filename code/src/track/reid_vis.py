...
class FramePlayerIDVisualizer:
    ...
    def analyze_data(self) -> Dict[str, Any]:
        ...
        results["multi_face_stats"] = {
            "player_combinations": Counter(),
            "per_trajectory": {}
        }
    ...
    def print_summary(self):
        ...
        print(f"Multi-face frames: {summary.get('multi_face_frames', 0)} ({summary.get('multi_face_rate', 0)*100:.1f}%)")
        ...
        if player_stats:
            ...
            for player, stats in sorted(player_stats.items(), key=lambda x: x[1]["total_frames"], reverse=True):
                print(f"{player}: {stats['total_frames']} frames ({stats['frame_ratio']*100:.1f}%), {stats['trajectories']} trajectories")
    ...
    def visualize_frame_trajectory_matrix(self, max_frames: int = 100, max_trajectories: int = 30):
        ...
        # Plot matrix
        im = ax1.imshow(matrix, aspect='auto', cmap=cmap, 
                       vmin=-1, vmax=len(player_codes),
        ...
        # Add title
        ax1.set_title(f'Frame-Trajectory Player Matching Matrix (Showing {len(sorted_trajectories)} trajectories, {len(sorted_frames)} frames)', 
        ...
        ax2.legend(handles=legend_patches, loc='center', 
                  ncol=min(6, len(legend_patches)), fontsize=9,
                  title='Player Color Legend', title_fontsize=10)
    ...
    def visualize_detailed_frame_analysis(self, selected_frames: List[int] = None):
        ...
        if selected_frames is None:
            ...
            if frame_match_counts:
                selected_frames = sorted(frame_match_counts.items(), 
    ...
    def _visualize_single_frame_details(self, frame_num: int):
        ...
        ax2.set_title(f'Frame {frame_num}: Detailed Player Matches', 
                     fontsize=12, fontweight='bold', pad=20)
        
        plt.suptitle(f'Frame {frame_num} - Trajectory-Player Matching Details', 
    ...
    def create_frame_match_timeline(self, selected_trajectories: List[str] = None, 
                                   max_frames: int = 200):
        """
        ...
        if selected_trajectories is None:
            ...
            top_trajectories = sorted(traj_frame_counts.items(), 
            ...
            for frame_num in sorted_frames:
                ...
                if player != prev_player or prev_frame is None:
                    ...
                    if prev_player is not None and segment_start is not None:
                        ...
                        ax.hlines(i, segment_start, prev_frame, 
        ...
        ax2.legend(handles=legend_elements, loc='upper left', 
                  ...
                  title='Player Colors', title_fontsize=10, fontsize=9)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--json_path', type=str, help='Path to frame_player_ids_*.json data file',
                        default="/data/ljy23/project/code/output/traj_reid/traj_reid/frame_player_ids_1200-3200frames.json")
    parser.add_argument('--output_dir', type=str, default="./player_id_matching_analysis", 
