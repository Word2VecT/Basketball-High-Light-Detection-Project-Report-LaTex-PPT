...
for frame_num in tqdm(range(min_frames), desc="读取帧"):
    ...
    frame_rgb_conv = cv2.cvtColor(combined_frame, cv2.COLOR_BGR2RGB)
    frames_list.append(frame_rgb_conv)

cap_rgb.release()
cap_skeleton.release()

print(f"\n[3/4] 使用imageio写入视频...")
writer = imageio.get_writer(output_path, fps=rgb_fps, macro_block_size=1)
