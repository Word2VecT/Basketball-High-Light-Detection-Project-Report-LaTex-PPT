import os
# 先设置CUDA环境变量
os.environ["CUDA_VISIBLE_DEVICES"] = "7,8,9"  # 使用的GPU编号
...
class ModelPool:
    ...
    def init_insightface_model(self):
        ...
        try:
            ...
            face_analyzer.prepare(ctx_id=0, det_size=(320, 320))  # ctx_id=1 表示使用 GPU
        finally:
            # 恢复标准输出和标准错误
    ...
    for seg_idx in range(segment_count):
        ...
        if os.path.exists(seg_output_dir) and os.path.exists(expected_merger_path):
            ...
            for i, vc in enumerate(video_configs):
                video_name = os.path.basename(vc["INPUT_VIDEO_PATH"]).replace(".mp4", "")
                output_path = os.path.join(seg_output_dir, f"output_{i}_{video_name}")
            ...
            # 7. 轨迹生成 + ReID（每个线程同时进行追踪和ReID）
            stage_name = f"片段{seg_idx + 1}/{segment_count} - 轨迹生成与ReID (帧{start_frame}~{end_frame})"
            ...
            merger = SerialTrajectoryMerger(
                all_json_paths=[os.path.join(p, "player_trajectory.json") for p in output_paths],
        ...
        # 9. 轨迹ReID可视化（使用之前的ReID结果）
        stage_name = f"片段{seg_idx + 1}/{segment_count} - 轨迹ReID可视化"
        log_stage_start(stage_name)
        reid_visualizer = TrajectoryReIDVisualizer(
            video_paths=VIDEO_PATHS,
            traj_path=merged_traj_path["merged_json"],
            reid_dirs=[os.path.join(p, "reid_results.json") for p in output_paths],
            output_dir=os.path.join(seg_output_dir, "traj_reid"),
            start_frame=start_frame,
